...
hostname=`hostname`
landevice=$(ip a |awk '/: e/{gsub(/:/,"");print $2}')
lanaddress=$(ip a s "$landevice" | awk '/inet /{gsub(/\/.*/,"");print $2}')
lanname=$(getent hosts "$lan" | awk '{print $2}')
ExternalIP=$(curl -s icanhazip.com)
Externalhostname=$(nslookup "$ExternalIP" | awk '{print $4}')
